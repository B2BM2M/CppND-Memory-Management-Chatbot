...
ChatBot::ChatBot(ChatBot &&source) // 4: right value - move constructor
{
    std::cout << "MOVING (c'tor) ChatBot instance" << &source << " to instance " << this  << std::endl;
    _image = source._image;
    _chatLogic = source._chatLogic;
    _rootNode = source._rootNode;
    _chatLogic->SetChatbotHandle(this); //set
    //_rootNode->SetChatbotHandle(this);
    source._image = NULL; // wxWidgets used NULL and not nullptr
