...
ChatLogic::ChatLogic() // chatlogic constructor, init
{ /* Jan3
    //// STUDENT CODE
    ////
...
ChatLogic::~ChatLogic() //destructor, out of scope functionality
{ /* Jan3
    //// STUDENT CODE
    ////
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                       ...
                       /// auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](GraphNode *node) { return node->GetID() == id; });
                        ...
                        {
                        /// Jan3    _nodes.emplace_back(new GraphNode(id));
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                           /// Jan3 GraphEdge *edge = new GraphEdge(id);
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id); // test
                          ///  edge->SetChildNode(*childNode);
                            edge->SetChildNode((*childNode).get());
                          ///  edge->SetParentNode(*parentNode);
                            edge->SetParentNode((*parentNode).get());
                          /// Jan3  _edges.push_back(edge); // 
    ...
    ChatBot chatbot(ChatBot("../images/chatbot.png")); //test
